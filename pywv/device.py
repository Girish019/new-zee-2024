...
from pywv.license_protocol_pb2 import ClientIdentification, FileHashes, SignedDrmCertificate, DrmCertificate
...
class Device:
    ...
    def __init__(
        ...
        **__: Any
        ...
        if not client_id:
            raise ValueError(
                "Client ID is required, the WVD does not contain one or is malformed.")
        if not private_key:
            raise ValueError(
                "Private Key is required, the WVD does not contain one or is malformed.")
        ...
        try:
            ...
            raise DecodeError(
                f"Failed to parse client_id as a ClientIdentification, {e}")
        ...
        if self.client_id.vmp_data:
            try:
                ...
                raise DecodeError(
                    f"Failed to parse Client ID's VMP data as a FileHashes, {e}")
        ...
        try:
            ...
            raise DecodeError(
                f"Failed to parse the DRM Certificate of the Client ID, {e}")
    ...
    def migrate(cls, data: Union[bytes, str]) -> Device:
        ...
        if header.version == 2:
            raise ValueError(
                "Device Data is already migrated to the latest version.")
        if header.version == 0 or header.version > 2:
            ...
            raise ValueError(
                "Device Data does not seem to be a WVD file (v0).")
        ...
        if header.version == 1:  # v1 to v2
            ...
            if data.vmp:
                try:
                    ...
                    raise DecodeError(
                        f"Failed to parse VMP data as FileHashes, {e}")
                data.vmp = vmp
                ...
                if client_id.vmp_data and client_id.vmp_data != new_vmp_data:
                    logging.getLogger("migrate").warning(

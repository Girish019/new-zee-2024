...
from pywv.exceptions import TooManySessions, InvalidSession, SignatureMismatch, InvalidInitData, \
...
async def open_(request: web.Request) -> web.Response:
    ...
    if not cdm:
        ...
        cdm = request.app["cdms"][(
            secret_key, device_name)] = Cdm.from_device(device)
...
async def get_service_certificate(request: web.Request) -> web.Response:
    ...
    if service_certificate:
        service_certificate = base64.b64encode(
            service_certificate.SerializeToString()).decode()
    ...
    if request.path != "/":
        ...
        if not secret_key:
            request.app.logger.debug(
                f"{request.remote} did not provide authorization.")
            ...
            request.app.logger.debug(
                f"{request.remote} failed authentication with '{secret_key}'.")
            response = web.json_response({
                "status": "401",

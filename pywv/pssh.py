...
from pywv.license_protocol_pb2 import WidevinePsshData
from pywv.utils import load_xml
...
class PSSH:
            ...
            if isinstance(data, str):
                try:
                    ...
                    raise binascii.Error(
                        f"Could not decode data as Base64, {e}")
    ...
    def new(
        ...
        if not isinstance(system_id, UUID):
            raise TypeError(
                f"Expected system_id to be a UUID, not {system_id!r}")
        ...
        if version not in (0, 1):
            raise ValueError(
                f"Invalid version, must be either 0 or 1, not {version}.")
        ...
        if key_ids is not None:
            ...
            if not all(isinstance(x, (UUID, bytes, str)) for x in key_ids):
                not_bytes = [x for x in key_ids if not isinstance(
                    x, (UUID, bytes, str))]
                raise TypeError(
        ...
        if self.system_id == PSSH.SystemId.PlayReady:
            ...
            for _ in range(pro_record_count):
                ...
                if prr_header_version == "4.0.0.0":
                    ...
                    key_ids = prr_header.xpath(
                        "DATA/PROTECTINFO/KIDS/KID/@VALUE")
                else:
                    raise ValueError(
    ...
    def set_key_ids(self, key_ids: list[UUID]) -> None:
        ...
        if not all(isinstance(x, UUID) for x in key_ids):
            ...
            raise TypeError(
                f"All Key IDs in key_ids must be a {UUID}, not {not_uuid}")

...
from pywv import __version__
from pywv.cdm import Cdm
from pywv.device import Device
from pywv.license_protocol_pb2 import LicenseType, FileHashes
...
def main(version: bool, debug: bool) -> None:
    ...
    log.info("pywidevine version %s Copyright (c) %s rlaphoenix",
             __version__, copyright_years)
...
def license_(device: Path, pssh: str, server: str, type_: str, privacy: bool):
    ...
    challenge = cdm.get_license_challenge(
        session_id, pssh, license_type, privacy_mode=True)
    ...
    if licence.status_code != 200:
        log.error("[-] Failed to send challenge: [%s] %s",
                  licence.status_code, licence.text)
        return
...
def create_device(
    ...
    if not key.is_file():
        raise click.UsageError(
            "key: Not a path to a file, or it doesn't exist.", ctx)
    if not client_id.is_file():
        raise click.UsageError(
            "client_id: Not a path to a file, or it doesn't exist.", ctx)
    if vmp and not vmp.is_file():
        raise click.UsageError(
            "vmp: Not a path to a file, or it doesn't exist.", ctx)
    ...
    out_path = (output or Path.cwd()) / \
        f"{name}_{device.system_id}_l{device.security_level}.wvd"
    ...
    log.info(" + Private Key: %s (%s bit)", bool(device.private_key),
             device.private_key.size_in_bits())
    log.info(" + Client ID: %s (%s bytes)", bool(device.client_id),
...
def export_device(ctx: click.Context, wvd_path: Path, out_dir: Optional[Path] = None) -> None:
    ...
    if not wvd_path.is_file():
        raise click.UsageError(
            "wvd_path: Not a path to a file, or it doesn't exist.", ctx)

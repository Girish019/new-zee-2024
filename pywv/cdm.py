...
from pywv.exceptions import TooManySessions, InvalidSession, InvalidLicenseType, SignatureMismatch, \
    InvalidInitData, InvalidLicenseMessage, NoKeysLoaded, InvalidContext
from pywv.key import Key
from pywv.license_protocol_pb2 import DrmCertificate, SignedMessage, SignedDrmCertificate, LicenseType, \
    LicenseRequest, ProtocolVersion, ClientIdentification, EncryptedClientIdentification, License
from pywv.pssh import PSSH
from pywv.session import Session
from pywv.utils import get_binary_path
...
class Cdm:
    ...
    def __init__(
        ...
        if not isinstance(rsa_key, RSA.RsaKey):
            raise TypeError(
                f"Expected rsa_key to be a {RSA.RsaKey} not {rsa_key!r}")
    ...
    def close(self, session_id: bytes) -> None:
        ...
        if not session:
            raise InvalidSession(
                f"Session identifier {session_id!r} is invalid.")
    ...
    def set_service_certificate(self, session_id: bytes, certificate: Optional[Union[bytes, str]]) -> str:
        ...
        if isinstance(certificate, str):
            ...
            raise DecodeError(
                f"Expecting Certificate to be bytes, not {certificate!r}")
    ...
    def get_license_challenge(
        ...
        license_request.content_id.widevine_pssh_data.pssh_data.append(
            pssh.init_data)
        ...
        license_message.signature = self.__signer.sign(
            SHA1.new(license_message.msg))
    ...
    def decrypt(
        ...
        if not isinstance(input_file, (Path, str)):
            raise ValueError(
                f"Expecting input_file to be a Path or str, got {input_file!r}")
        if not isinstance(output_file, (Path, str)):
            raise ValueError(
                f"Expecting output_file to be a Path or str, got {output_file!r}")
        if not isinstance(temp_dir, (Path, str)) and temp_dir is not None:
            raise ValueError(
                f"Expecting temp_dir to be a Path or str, got {temp_dir!r}")
        ...
        platform = {"win32": "win", "darwin": "osx"}.get(

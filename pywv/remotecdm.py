...
from pywv.exceptions import InvalidInitData, InvalidLicenseType, InvalidLicenseMessage, DeviceMismatch, \
    SignatureMismatch
from pywv.key import Key

from pywv.license_protocol_pb2 import LicenseType, SignedMessage, License, ClientIdentification, \
...
class RemoteCdm(Cdm):
    ...
    def __init__(
        ...
        if not isinstance(system_id, int):
            raise TypeError(
                f"Expected system_id to be a {int} not {system_id!r}")

        if not security_level:
            raise ValueError("Security Level must be provided")
        if not isinstance(security_level, int):
            raise TypeError(
                f"Expected security_level to be a {int} not {security_level!r}")
        ...
        if not isinstance(device_name, str):
            raise TypeError(
                f"Expected device_name to be a {str} not {device_name!r}")
        ...
        super().__init__(device_type, system_id, security_level,
                         ClientIdentification(), RSA.generate(2048))
        ...
        if r.status_code != 200:
            raise ValueError(
                f"Could not test Remote API version [{r.status_code}]")
        server = r.headers.get("Server")
        if not server or "pywidevine serve" not in server.lower():
            raise ValueError(
                f"This Remote CDM API does not seem to be a pywidevine serve API ({server}).")
        server_version = re.search(
            r"pywidevine serve v([\d.]+)", server, re.IGNORECASE)
        if not server_version:
            raise ValueError(
                "The pywidevine server API is not stating the version correctly, cannot continue.")
        server_version = server_version.group(1)
        if server_version < "1.4.3":
            raise ValueError(
                f"This pywidevine serve API version ({server_version}) is not supported.")

    @classmethod
    def from_device(cls, device: Device) -> RemoteCdm:
        raise NotImplementedError(
            "You cannot load a RemoteCdm from a local Device file.")
    ...
    def open(self) -> bytes:
        ...
        if r['status'] != 200:
            raise ValueError(
                f"Cannot Open CDM Session, {r['message']} [{r['status']}]")
        r = r["data"]
    ...
    def close(self, session_id: bytes) -> None:
        ...
        if r["status"] != 200:
            raise ValueError(
                f"Cannot Close CDM Session, {r['message']} [{r['status']}]")
    ...
    def get_service_certificate(self, session_id: bytes) -> Optional[SignedDrmCertificate]:
        ...
        try:
            ...
            raise DecodeError(
                f"Could not parse certificate as a SignedDrmCertificate, {e}")
    ...
    def get_license_challenge(
        ...
        if not isinstance(pssh, PSSH):
            raise InvalidInitData(
                f"Expected pssh to be a {PSSH}, not {pssh!r}")
        ...
        if r["status"] != 200:
            raise ValueError(
                f"Cannot get Challenge, {r['message']} [{r['status']}]")
        r = r["data"]
        ...
        try:
            ...
            raise InvalidLicenseMessage(
                f"Failed to parse license request, {e}")
    ...
    def parse_license(self, session_id: bytes, license_message: Union[SignedMessage, bytes, str]) -> None:
        ...
        if isinstance(license_message, str):
            try:
                ...
                raise InvalidLicenseMessage(
                    f"Could not decode license_message as Base64, {e}")
        ...
        if isinstance(license_message, bytes):
            ...
            try:
                ...
                raise InvalidLicenseMessage(
                    f"Could not parse license_message as a SignedMessage, {e}")
        ...
        if r["status"] != 200:
            raise ValueError(
                f"Cannot parse License, {r['message']} [{r['status']}]")
    ...
    def get_keys(self, session_id: bytes, type_: Optional[Union[int, str]] = None) -> list[Key]:
        try:
            ...
            raise ValueError(
                f"Could not parse type_ as a {License.KeyContainer.KeyType}, {e}")
        ...
        if r["status"] != 200:
            raise ValueError(
                f"Could not get {type_} Keys, {r['message']} [{r['status']}]")
        r = r["data"]
